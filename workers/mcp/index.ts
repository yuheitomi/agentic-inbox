...
/**
 * Build the email MCP server — exposes email tools over the Model Context Protocol.
 *
 ...
 *
 * The handler is stateless: `createMcpHandler` calls this factory once per
 * request, so `env` is closed over rather than read off an instance.
 */
export function createEmailMcpServer(env: Env) {
